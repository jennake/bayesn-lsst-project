...
def plot_one(model, snid, status, max_rhat, head_row, phot_data):
    ...
    model_bands = [LSST_FILT_MAP[b] for b in observed_bands]

    phases = np.linspace(-10, 40, 200)

    ebv_mw = float(head_row["MWEBV"])

    # Posterior redshift median, since these fits used photoz=True
    ...
    phot_grid = model.get_flux_from_chains(
        ...
        bands=model_bands,
        chains=chains,
        zs=np.array([z_med]) if z_med is not None else None,
    ...
    peak_mjd = float(np.median(np.asarray(chains["peak_MJD"])))
    z_for_time = z_med if z_med is not None else 0.0
    model_mjd = peak_mjd + phases * (1.0 + z_for_time)
    ...
    ax.set_title(f"{snid}  [{status}, max r_hat={rhat_str}]")
